...
max_number_of_principal_components = (
    20  # 使用する主成分の最大数。説明変数の数より小さい必要があります
)
...
for i in range(original_x.shape[1]):
    for j in range(original_x.shape[1]):
        if i == j:  # 二乗項
            ...
            x = x.rename(
                columns={0: "{0}*{1}".format(x_square.columns[i], x_square.columns[j])}
            )
...
standard_regression_coefficients = pd.DataFrame(
    model.coef_[0], index=x_train.columns, columns=["standard_regression_coefficients"]
