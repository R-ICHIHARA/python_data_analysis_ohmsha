...
from sklearn.model_selection import (  # クロスバリデーションにより推定値を計算するために使用
    cross_val_predict,
    train_test_split,
)
...
max_number_of_principal_components = (
    13  # 使用する主成分の最大数。説明変数の数より小さい必要があります
)
...
t_train = pd.DataFrame(
    pca.transform(autoscaled_x_train)
)  # トレーニングデータの主成分スコアの計算
t_train.index = x_train.index
t_test = pd.DataFrame(
    pca.transform(autoscaled_x_test)
)  # テストデータの主成分スコアの計算
...
for component in range(1, max_number_of_principal_components + 1):
    ...
    estimated_y_in_cv = pd.DataFrame(
        cross_val_predict(
            model, t_train.iloc[:, 0:component], autoscaled_y_train, cv=fold_number
        )
...
sample_functions.estimation_and_performance_check_in_regression_train_and_test(
    model, t_train, y_train, t_test, y_test
)
