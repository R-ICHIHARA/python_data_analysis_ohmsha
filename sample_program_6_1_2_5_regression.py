...
if add_nonlinear_terms_flag:
    x = pd.read_csv("./sample_data/x_superconductor.csv", index_col=0)
...
optimal_ocsvm_gamma = sample_functions.gamma_optimization_with_variance(
    autoscaled_x, ocsvm_gammas
)
...
if method_name == "pls":
    ...
    standard_regression_coefficients = pd.DataFrame(
        model.coef_[0], index=x.columns, columns=["standard_regression_coefficients"]
    )
    standard_regression_coefficients.to_csv(
        "./output/pls_standard_regression_coefficients.csv"
    )  # csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください

# OCSVM による AD
print("最適化された gamma (OCSVM) :", optimal_ocsvm_gamma)
ad_model = svm.OneClassSVM(
    kernel="rbf", gamma=optimal_ocsvm_gamma, nu=ocsvm_nu
)  # AD モデルの宣言
...
for iteration in range(number_of_iterations):
    ...
    random_numbers = np.random.rand(
        number_of_samples_in_prediction, number_of_atom_types * 2
    )
    random_numbers[:, 0:number_of_atom_types] = (
        np.ceil(random_numbers[:, 0:number_of_atom_types] * original_x.shape[1]) - 1
    )
    ...
    original_x_prediction = np.zeros(
        [number_of_samples_in_prediction, original_x.shape[1]]
    )
    for sample_number in range(number_of_samples_in_prediction):
        values_of_atoms = (
            random_numbers[sample_number, number_of_atom_types:]
            * upper_x.iloc[random_numbers[sample_number, 0:number_of_atom_types]].values
        )
        original_x_prediction[
            sample_number,
            random_numbers[sample_number, 0:number_of_atom_types].astype(np.int64),
        ] = values_of_atoms
    ...
    autoscaled_original_x_prediction = (
        original_x_prediction - original_x.mean()
    ) / original_x.std()
    data_density_prediction = ad_model.decision_function(
        autoscaled_original_x_prediction
    )  # データ密度 (f(x) の値)
    original_x_prediction_inside_ad = original_x_prediction.iloc[
        data_density_prediction >= 0, :
    ]
    original_x_prediction_inside_ad = original_x_prediction_inside_ad.reset_index(
        drop=True
    )

    if add_nonlinear_terms_flag:
        x_prediction = sample_functions.add_nonlinear_terms(
            original_x_prediction_inside_ad
        )  # 説明変数の二乗項や交差項を追加
        x_prediction = x_prediction.drop(
            x_prediction.columns[std_0_nonlinear_variable_flags], axis=1
        )  # 標準偏差が 0 の説明変数を削除
    else:
    ...
    predicted_y = pd.DataFrame(
        model.predict(autoscaled_x_prediction) * y.std() + y.mean()
    )
...
predicted_dataset.to_csv(
    "./output/predicted_critical_temp_dataset.csv"
)  # csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください
