...
if add_nonlinear_terms_flag:
    ...
    x_train = x_train.drop(
        x_train.columns[std_0_nonlinear_variable_flags], axis=1
    )  # 標準偏差が 0 の説明変数を削除
    x_test = x_test.drop(
        x_test.columns[std_0_nonlinear_variable_flags], axis=1
    )  # 標準偏差が 0 の説明変数を削除
...
if method_name == "knn":
    ...
    for k in range(1, max_number_of_k + 1):
        ...
        estimated_y_in_cv = pd.DataFrame(
            cross_val_predict(model, autoscaled_x_train, y_train, cv=fold_number)
        )
        accuracy_in_cv = metrics.accuracy_score(
            y_train, estimated_y_in_cv
        )  # 正解率を計算
    ...
    optimal_svm_gamma = sample_functions.gamma_optimization_with_variance(
    ...
    for index, x_variables_rate in enumerate(rf_x_variables_rates):
        ...
        model_in_validation = RandomForestClassifier(
            ...
            max_features=int(
                max(math.ceil(autoscaled_x_train.shape[1] * x_variables_rate), 1)
            ),
            oob_score=True,
        )
