...
x_train, x_test, y_train, y_test = train_test_split(
    x, y, test_size=number_of_test_samples, shuffle=True, random_state=21
)
# 標準偏差が 0 の説明変数を削除
...
if method_name == "pls":
    ...
    for component in range(
        1,
        min(
            np.linalg.matrix_rank(autoscaled_x_train),
        ...
        estimated_y_in_cv = pd.DataFrame(
            cross_val_predict(
                model, autoscaled_x_train, autoscaled_y_train, cv=fold_number
            )
        )  # クロスバリデーション推定値の計算し、DataFrame型に変換
        estimated_y_in_cv = (
            estimated_y_in_cv * y_train.std() + y_train.mean()
        )  # スケールをもとに戻す
