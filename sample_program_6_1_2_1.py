...
correlation_coefficients.to_csv(
    "./output/correlation_coefficients.csv"
)  # 相関行列を csv ファイルとして保存
# 相関行列のヒートマップ (相関係数の値なし)
plt.rcParams["font.size"] = 12
sns.heatmap(
    correlation_coefficients, vmax=1, vmin=-1, cmap="seismic", square=True, annot=False
)
...
plt.rcParams["font.size"] = 18  # 横軸や縦軸の名前の文字などのフォントのサイズ
plt.hist(
    correlation_coefficients.iloc[1:, 0], bins=number_of_bins
)  # ヒストグラムの作成
...
plt.rcParams["font.size"] = 18  # 横軸や縦軸の名前の文字などのフォントのサイズ
plt.scatter(
    new_dataset.iloc[:, variable_number_1],
    new_dataset.iloc[:, variable_number_2],
    c="blue",
)
