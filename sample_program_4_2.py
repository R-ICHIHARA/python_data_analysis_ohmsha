...
x_new = dataset.iloc[
    :, 1:
]  # 今回はモデル構築に用いたデータセットと同じデータセットの y を推定します
...
estimated_y_new = pd.DataFrame(
    estimated_y_new, index=dataset.index, columns=["estimated_y"]
)
estimated_y_new.to_csv(
    "./output/estimated_y.csv"
...
y_max = max(
    y.max(), estimated_y_new.iloc[:, 0].max()
)  # 実測値の最大値と、推定値の最大値の中で、より大きい値を取得
y_min = min(
    y.min(), estimated_y_new.iloc[:, 0].min()
...
standard_regression_coefficients.to_csv(
    "./output/standard_regression_coefficients.csv"
