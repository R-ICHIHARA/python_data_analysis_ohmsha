...
rate_of_training_samples_inside_ad = (
    0.68  # AD 内となるトレーニングデータの割合。AD　のしきい値を決めるときに使用
)
...
if method_name != "pls" and method_name != "svr":
    sys.exit(
        "'{0}' という回帰分析手法はありません。method_name を見直してください。".format(
            method_name
        )
    )
...
if add_nonlinear_terms_flag:
    x_tmp = sample_functions.add_nonlinear_terms(
        dataset.iloc[:, number_of_y:]
    )  # 説明変数の二乗項や交差項を追加
    x = x_tmp.drop(
        x_tmp.columns[x_tmp.std() == 0], axis=1
    )  # 標準偏差が 0 の説明変数を削除
else:
...
for y_number in range(number_of_y):
    ...
    if method_name == "pls":
        ...
        components = (
            []
        )  # 空の list の変数を作成して、成分数をこの変数に追加していきます同じく成分数をこの変数に追加
        r2_in_cv_all = (
            []
        )  # 空の list の変数を作成して、成分数ごとのクロスバリデーション後の r2 をこの変数に追加
        for component in range(
            1,
            min(np.linalg.matrix_rank(autoscaled_x), max_number_of_principal_components)
            + 1,
        ):
            # PLS
            model = PLSRegression(n_components=component)  # PLS モデルの宣言
            estimated_y_in_cv = pd.DataFrame(
                cross_val_predict(model, autoscaled_x, autoscaled_y, cv=fold_number)
            )  # クロスバリデーション推定値の計算し、DataFrame型に変換
            estimated_y_in_cv = (
                estimated_y_in_cv * y.std() + y.mean()
            )  # スケールをもとに戻す
        ...
        optimal_component_number = (
            sample_functions.plot_and_selection_of_hyperparameter(
        ...
        optimal_svr_gamma = sample_functions.gamma_optimization_with_variance(
            autoscaled_x, svr_gammas
        )
        # CV による ε の最適化
        ...
        # CV による γ の最適化
        model_in_cv = GridSearchCV(
            svm.SVR(kernel="rbf", epsilon=optimal_svr_epsilon, C=optimal_svr_c),
            {"gamma": svr_gammas},
            cv=fold_number,
            iid=False,
        )
        ...
        print(
            "C : {0}\nε : {1}\nGamma : {2}".format(
                optimal_svr_c, optimal_svr_epsilon, optimal_svr_gamma
            )
        )
        # SVR
        model = svm.SVR(
    ...
    if method_name == "pls":
        ...
        standard_regression_coefficients = pd.DataFrame(
            model.coef_[0]
        )  # Pandas の DataFrame 型に変換
        standard_regression_coefficients.index = (
            x.columns
        )  # 説明変数に対応する名前を、元のデータセットにおける説明変数の名前に
        standard_regression_coefficients.columns = [
            "standard_regression_coefficients"
        ]  # 列名を変更
        standard_regression_coefficients.to_csv(
            "pls_standard_regression_coefficients_y{0}.csv".format(y_number)
...
mean_of_knn_distance_train = pd.DataFrame(
    knn_distance_train.iloc[:, 1:].mean(axis=1)
)  # 自分以外の k_in_knn 個の距離の平均。Series 型のため、列名の設定は別に
...
sorted_mean_of_knn_distance_train = mean_of_knn_distance_train.iloc[:, 0].sort_values(
    ascending=True
)  # 距離の平均の小さい順に並び替え
...
# 新しいサンプルにおける予測
dataset_prediction = pd.read_csv(
    "./sample_data/virtual_resin_prediction.csv", encoding="SHIFT-JIS", index_col=0
)
if add_nonlinear_terms_flag:
    x_prediction_tmp = sample_functions.add_nonlinear_terms(
        dataset_prediction
    )  # 説明変数の二乗項や交差項を追加
    x_prediction = x_prediction_tmp.drop(
        x_tmp.columns[x_tmp.std() == 0], axis=1
    )  # 標準偏差が 0 の説明変数を削除
else:
...
for y_number in range(number_of_y):
    ...
    predicted_y = pd.DataFrame(
        autoscaled_predicted_y * ys.iloc[:, y_number].std()
        + ys.iloc[:, y_number].mean()
    )
...
knn_distance_prediction, knn_index_prediction = ad_model.kneighbors(
    autoscaled_x_prediction, n_neighbors=k_in_knn
)
knn_distance_prediction = pd.DataFrame(
    knn_distance_prediction, index=x_prediction.index
)
knn_distance_prediction.index = (
    x_prediction.index
)  # サンプル名をトレーニングデータのサンプル名に
mean_of_knn_distance_prediction = pd.DataFrame(
    knn_distance_prediction.mean(axis=1)
)  # k_in_knn 個の距離の平均
inside_ad_flag_prediction = (
    mean_of_knn_distance_prediction <= ad_threshold
)  # AD 内のサンプルのみ TRUE
inside_ad_flag_prediction.columns = ["inside_ad_flag"]  # 列名を変更
inside_ad_flag_prediction.to_csv(
    "inside_ad_flag_prediction.csv"
)  # csv ファイルに保存。同じ名前のファイルがあるときは上書きされるため注意

# パレート最適解の探索
print("\nパレート最適解の探索")
predicted_ys_inside_ad = predicted_ys.iloc[inside_ad_flag_prediction.values[:, 0], :]
dataset_prediction_inside_ad = dataset_prediction.iloc[
    inside_ad_flag_prediction.values[:, 0], :
]
...
samples_inside_ad = pd.concat(
    [predicted_ys_inside_ad, dataset_prediction_inside_ad], axis=1
)
pareto_optimal_samples = samples_inside_ad.iloc[pareto_optimal_index, :]
pareto_optimal_samples.to_csv(
    "pareto_optimal_samples_prediction.csv"
)  # csv ファイルに保存。同じ名前のファイルがあるときは上書きされるため注意
...
plt.scatter(
    ...
    predicted_ys.iloc[:, 1],
    color="0.75",
    label="samples outside AD in prediction",
)
plt.scatter(
    predicted_ys.iloc[inside_ad_flag_prediction.values[:, 0], 0],
    predicted_ys.iloc[inside_ad_flag_prediction.values[:, 0], 1],
    color="black",
    label="samples inside AD in prediction",
)
plt.scatter(
    predicted_ys_inside_ad.iloc[pareto_optimal_index, 0],
    predicted_ys_inside_ad.iloc[pareto_optimal_index, 1],
    color="blue",
