...
def k3n_error(x_1, x_2, k):
    ...
    knn_distance_in_x_1 = np.sort(
        x_2_distance[:, x_1_sorted_indexes[:, 1 : k + 1]][identity_matrix]
    )
...
def plot_and_selection_of_hyperparameter(
    hyperparameter_values, metrics_values, x_label, y_label
):
...
def estimation_and_performance_check_in_regression_train_and_test(
    model, x_train, y_train, x_test, y_test
):
    # トレーニングデータの推定
    estimated_y_train = (
        model.predict(x_train) * y_train.std() + y_train.mean()
    )  # y を推定し、スケールをもとに戻します
    estimated_y_train = pd.DataFrame(
        estimated_y_train, index=x_train.index, columns=["estimated y"]
    ...
    print(
        ...
        metrics.mean_squared_error(y_train, estimated_y_train) ** 0.5,
    )
    print(
        "MAE for training data :",
        metrics.mean_absolute_error(y_train, estimated_y_train),
    )
    ...
    results_train = pd.concat(
        [estimated_y_train, y_train_for_save, y_error_train], axis=1
    )
    results_train.to_csv(
        "estimated_y_train.csv"
    )  # 推定値を csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください

    # テストデータの推定
    estimated_y_test = (
        model.predict(x_test) * y_train.std() + y_train.mean()
    )  # y を推定し、スケールをもとに戻します
    estimated_y_test = pd.DataFrame(
        estimated_y_test, index=x_test.index, columns=["estimated y"]
    )  # Pandas の DataFrame 型に変換。行の名前・列の名前も設定
    ...
    plt.figure(figsize=figure.figaspect(1))  # 図の形を正方形に
    plt.scatter(
        y_test, estimated_y_test.iloc[:, 0], c="blue"
    )  # 実測値 vs. 推定値プロット
    y_max = max(
    ...
    print(
        "RMSE for test data :",
        metrics.mean_squared_error(y_test, estimated_y_test) ** 0.5,
    )
    ...
    results_test.to_csv(
        "estimated_y_test.csv"
    )  # 推定値を csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください


def estimation_and_performance_check_in_classification_train_and_test(
    model, x_train, y_train, x_test, y_test
):
    class_types = list(
        set(y_train)
    )  # クラスの種類。これで混同行列における縦と横のクラスの順番を定めます
    class_types.sort(reverse=True)  # 並び替え

    # トレーニングデータのクラスの推定
    estimated_y_train = pd.DataFrame(
        model.predict(x_train), index=x_train.index, columns=["estimated class"]
    )  # トレーニングデータのクラスを推定し、Pandas の DataFrame 型に変換。行の名前・列の名前も設定

    # トレーニングデータの混同行列
    confusion_matrix_train = pd.DataFrame(
        metrics.confusion_matrix(y_train, estimated_y_train, labels=class_types),
        index=class_types,
        columns=class_types,
    )  # 混同行列を作成し、Pandas の DataFrame 型に変換。行の名前・列の名前を定めたクラスの名前として設定
    confusion_matrix_train.to_csv(
        "confusion_matrix_train.csv"
    )  # csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください
    print(confusion_matrix_train)  # 混同行列の表示
    print(
        "Accuracy for training data :",
        metrics.accuracy_score(y_train, estimated_y_train),
        "\n",
    )  # 正解率の表示
    ...
    # テストデータのクラスの推定
    estimated_y_test = pd.DataFrame(
        model.predict(x_test), index=x_test.index, columns=["estimated class"]
    )  # テストデータのクラスを推定し、Pandas の DataFrame 型に変換。行の名前・列の名前も設定

    # テストデータの混同行列
    confusion_matrix_test = pd.DataFrame(
        metrics.confusion_matrix(y_test, estimated_y_test, labels=class_types),
        index=class_types,
        columns=class_types,
    )  # 混同行列を作成し、Pandas の DataFrame 型に変換。行の名前・列の名前を定めたクラスの名前として設定
    confusion_matrix_test.to_csv(
        "confusion_matrix_test.csv"
    )  # csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください
    print(confusion_matrix_test)  # 混同行列の表示
    print(
        "Accuracy for test data :", metrics.accuracy_score(y_test, estimated_y_test)
    )  # 正解率の表示
            ...
            if i == j:  # 二乗項
                x = pd.concat(
                    [
                        ...
                        x_square.rename(
                            columns={
                                x_square.columns[i]: "{0}^2".format(x_square.columns[i])
                            }
                        ).iloc[:, i],
                    ],
                    axis=1,
                ...
                x_cross.name = "{0}*{1}".format(
                    original_x.columns[i], original_x.columns[j]
                )
...
def add_time_delayed_variable(x, dynamics_max, dynamics_span):
    ...
    if dynamics_max:
        ...
        x_with_dynamics = np.append(
            x_with_dynamics, x_array[dynamics_max:, 0:1], axis=1
        )
        for x_variable_number in range(x_array.shape[1] - 1):
            x_with_dynamics = np.append(
                x_with_dynamics,
                x_array[dynamics_max:, x_variable_number + 1 : x_variable_number + 2],
                axis=1,
            )
            for time_delay_number in range(int(np.floor(dynamics_max / dynamics_span))):
                x_with_dynamics = np.append(
                    ...
                    x_array[
                        dynamics_max
                        - (time_delay_number + 1)
                        * dynamics_span : -(time_delay_number + 1)
                        * dynamics_span,
...
def lwpls(x_train, y_train, x_test, max_component_number, lambda_in_similarity):
    ...
    for test_sample_number in range(x_test.shape[0]):
        ...
        similarity = np.diag(
            np.exp(-distance / distance.std(ddof=1) / lambda_in_similarity)
        )
        #        similarity_matrix = np.diag(similarity)

        y_w = y_train.T.dot(np.diag(similarity)) / similarity.sum()
        x_w = np.reshape(
            x_train.T.dot(np.diag(similarity)) / similarity.sum(), (1, x_train.shape[1])
        )
        ...
        for component_number in range(max_component_number):
            w_a = np.reshape(
                centered_x.T.dot(similarity).dot(centered_y)
                / np.linalg.norm(centered_x.T.dot(similarity).dot(centered_y)),
                (x_train.shape[1], 1),
            )
            t_a = np.reshape(centered_x.dot(w_a), (x_train.shape[0], 1))
            p_a = np.reshape(
                centered_x.T.dot(similarity).dot(t_a) / t_a.T.dot(similarity).dot(t_a),
                (x_train.shape[1], 1),
            )
            q_a = centered_y.T.dot(similarity).dot(t_a) / t_a.T.dot(similarity).dot(t_a)
            t_q_a = centered_query_x_test.dot(w_a)
            estimated_y_test[test_sample_number, component_number:] = (
                estimated_y_test[test_sample_number, component_number:] + t_q_a * q_a
            )
...
def structure_generation_based_on_r_group_random(
    file_name_of_main_fragments, file_name_of_sub_fragments, number_of_structures
):
    ...
    main_molecules = [
        molecule
        for molecule in Chem.SmilesMolSupplier(
    ...
    for generated_structure_number in range(number_of_structures):
        ...
        for r_number_in_molecule in range(len(r_index_in_main_molecule_new)):
            ...
            generated_adjacency_matrix[main_size:, main_size:] = (
                fragment_adjacency_matrix
            )
        ...
        if (generated_structure_number + 1) % 1000 == 0 or (
