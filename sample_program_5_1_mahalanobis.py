...
rate_of_training_samples_inside_ad = (
    0.8  # AD 内となるトレーニングデータの割合。AD　のしきい値を決めるときに使用
)
...
# k-NN による AD
ad_model = NearestNeighbors(
    n_neighbors=k_in_knn,
    metric="mahalanobis",
    metric_params={"V": autoscaled_x_train.cov()},
)  # AD モデルの宣言
ad_model.fit(
    autoscaled_x_train
...
knn_distance_train, knn_index_train = ad_model.kneighbors(
    autoscaled_x_train, n_neighbors=k_in_knn + 1
)
...
mean_of_knn_distance_train.to_csv(
    "mean_of_knn_distance_train.csv"
...
inside_ad_flag_train = (
    mean_of_knn_distance_train <= ad_threshold
)  # AD 内のサンプルのみ TRUE
inside_ad_flag_train.columns = ["inside_ad_flag"]
inside_ad_flag_train.to_csv(
    "inside_ad_flag_train.csv"
)  # csv ファイルに保存。同じ名前のファイルがあるときは上書きされるため注意

# テストデータに対する k-NN 距離の計算
knn_distance_test, knn_index_test = ad_model.kneighbors(autoscaled_x_test)
knn_distance_test = pd.DataFrame(
    knn_distance_test, index=x_test.index
)  # DataFrame型に変換
mean_of_knn_distance_test = pd.DataFrame(
    knn_distance_test.mean(axis=1), columns=["mean_of_knn_distance"]
)  # k_in_knn 個の距離の平均
mean_of_knn_distance_test.to_csv(
    "mean_of_knn_distance_test.csv"
)  # csv ファイルに保存。同じ名前のファイルがあるときは上書きされるため注意

# テストデータに対して、AD の中か外かを判定
inside_ad_flag_test = (
    mean_of_knn_distance_test <= ad_threshold
)  # AD 内のサンプルのみ TRUE
inside_ad_flag_test.columns = ["inside_ad_flag"]
inside_ad_flag_test.to_csv(
    "./output/inside_ad_flag_test.csv"
)  # csv ファイルに保存。同じ名前のファイルがあるときは上書きされるため注意
