...
rf_x_variables_rates = (
    np.arange(1, 11, dtype=float) / 10
)  # 1 つの決定木における説明変数の数の割合の候補
...
if method_name != "knn" and method_name != "svm" and method_name != "rf":
    sys.exit(
        "'{0}' というクラス分類手法はありません。method_name を見直してください。".format(
            method_name
        )
    )
...
y[dataset.iloc[:, 86] >= 90] = (
    "positive"  # 転移温度 90 K 以上を高温超伝導体 (positive) とします
)
...
autoscaled_original_x = (
    original_x - original_x.mean()
) / original_x.std()  # オートスケーリング
...
if method_name == "knn":
    ...
    # k-NN
    model = KNeighborsClassifier(
        n_neighbors=optimal_k, metric="euclidean"
    )  # モデルの宣言
    ...
    # SVM
    model = svm.SVC(
        kernel="rbf", C=optimal_svm_c, gamma=optimal_svm_gamma
    )  # モデルの宣言
elif method_name == "rf":
    ...
    for index, x_variables_rate in enumerate(rf_x_variables_rates):
        ...
        model_in_validation = RandomForestClassifier(
            ...
            max_features=int(
                max(math.ceil(autoscaled_x.shape[1] * x_variables_rate), 1)
            ),
            oob_score=True,
        )
...
for iteration in range(number_of_iterations):
    ...
    predicted_positive_dataset = pd.concat(
        [
            predicted_positive_dataset,
            original_x_prediction_inside_ad.iloc[predicted_y == "positive", :],
        ],
        axis=0,
    )

predicted_positive_dataset = predicted_positive_dataset.reset_index(drop=True)
predicted_positive_dataset.to_csv(
    "./output/predicted_positive_dataset.csv"
)  # csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください
