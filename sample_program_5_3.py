...
# テストデータの y の推定
estimated_y_test_all = (
    pd.DataFrame()
)  # 空の DataFrame 型を作成し、ここにサブモデルごとのテストデータの y の推定結果を追加
for submodel_number in range(number_of_submodels):
    # 説明変数の選択
    ...
    estimated_y_test = (
        estimated_y_test * y_train.std() + y_train.mean()
    )  # スケールをもとに戻します
...
# テストデータの推定値の標準偏差
std_of_estimated_y_test = pd.DataFrame(
...
std_of_estimated_y_test.to_csv(
    "./output/std_of_estimated_y_test.csv"
)  # 推定値の標準偏差を csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください

# テストデータの推定値の標準偏差 vs. 推定値の誤差の絶対値プロット
plt.rcParams["font.size"] = 18  # 横軸や縦軸の名前の文字などのフォントのサイズ
plt.scatter(
    std_of_estimated_y_test.iloc[:, 0],
    abs(y_test - estimated_y_test.iloc[:, 0]),
    c="blue",
)  # 実測値 vs. 推定値プロット
