...
import pandas as pd
from scipy.cluster.hierarchy import (  # SciPy の中の階層的クラスタリングを実行したり樹形図を作成したりするためのライブラリをインポート
    dendrogram,
    fcluster,
    linkage,
)
...
dendrogram(
    clustering_results, labels=list(x.index), color_threshold=0, orientation="right"
)  # デンドログラムの作成。labels=x.index でサンプル名を入れています
plt.xlabel("distance")  # 横軸の名前
plt.show()

# クラスター番号の保存
cluster_numbers = fcluster(
    clustering_results, number_of_clusters, criterion="maxclust"
)  # クラスターの数で分割し、クラスター番号を出力
cluster_numbers = pd.DataFrame(
    cluster_numbers, index=x.index, columns=["cluster_numbers"]
)  # DataFrame 型に変換。行の名前・列の名前も設定
cluster_numbers.to_csv("./output/cluster_numbers.csv")
