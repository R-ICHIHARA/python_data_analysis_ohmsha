...
for ocsvm_gamma in ocsvm_gammas:
    gram_matrix = np.exp(
        -ocsvm_gamma
        * cdist(autoscaled_x_train, autoscaled_x_train, metric="seuclidean")
    )
    variance_of_gram_matrix.append(gram_matrix.var(ddof=1))
optimal_ocsvm_gamma = ocsvm_gammas[
...
print(
    ...
    number_of_support_vectors / x_train.shape[0],
)
print(
...
print(
    ...
    number_of_outliers_in_training_data / x_train.shape[0],
)
data_density_train = pd.DataFrame(
    data_density_train, index=x_train.index, columns=["ocsvm_data_density"]
)
data_density_train.to_csv(
    "ocsvm_data_density_train.csv"
...
inside_ad_flag_train.to_csv(
    "./output/inside_ad_flag_train.csv"
)  # csv ファイルに保存。同じ名前のファイルがあるときは上書きされるため注意
...
print(
    "テストデータにおける外れサンプルの割合 :",
    number_of_outliers_in_test_data / x_test.shape[0],
)
data_density_test = pd.DataFrame(
    data_density_test, index=x_test.index, columns=["ocsvm_data_density"]
)
data_density_test.to_csv(
    "./output/ocsvm_data_density_test.csv"
)  # csv ファイルに保存。同じ名前のファイルがあるときは上書きされるため注意
