...
rate_of_selected_x_variables = (
    0.7  # 各サブデータセットで選択される説明変数の数の割合。0 より大きく 1 未満
)
...
x_train, x_test, y_train, y_test = train_test_split(
    x, y, test_size=number_of_test_samples, shuffle=True, random_state=21, stratify=y
)
...
estimated_y_train_all = (
    pd.DataFrame()
)  # 空の DataFrame 型を作成し、ここにサブモデルごとのトレーニングデータの y の推定結果を追加
selected_x_variable_numbers = (
    []
)  # 空の list 型の変数を作成し、ここに各サブデータセットの説明変数の番号を追加
...
for submodel_number in range(number_of_submodels):
    ...
    model_in_cv = GridSearchCV(
        svm.SVC(kernel="rbf"), {"C": svm_cs, "gamma": svm_gammas}, cv=fold_number
    )
...
estimated_y_test_count = np.zeros(
    [x_test.shape[0], len(class_types)]
)  # クラスごとに、推定したサブモデルの数をカウントして値をここに格納
for submodel_number in range(number_of_submodels):
    # 説明変数の選択
    selected_autoscaled_x_test = autoscaled_x_test.iloc[
        :, selected_x_variable_numbers[submodel_number]
    ]

    # テストデータの y の推定
    estimated_y_test = pd.DataFrame(
        submodels[submodel_number].predict(selected_autoscaled_x_test)
    )  # テストデータの y の値を推定し、Pandas の DataFrame 型に変換
    ...
    for sample_number in range(estimated_y_test.shape[0]):
        estimated_y_test_count[
            sample_number, class_types.index(estimated_y_test.iloc[sample_number, 0])
        ] += 1

# テストデータにおける、クラスごとの推定したサブモデルの数
estimated_y_test_count = pd.DataFrame(
    estimated_y_test_count, index=x_test.index, columns=class_types
)
estimated_y_test_count.to_csv(
    "./output/estimated_y_test_count.csv"
...
estimated_y_test_probability.to_csv(
    "./output/estimated_y_test_probability.csv"
)  # csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください

# テストデータにおける、多数決で推定された結果
estimated_y_test = pd.DataFrame(
    estimated_y_test_count.idxmax(axis=1), columns=["estimated_class"]
)
estimated_y_test.to_csv(
    "./output/estimated_y_test.csv"
)  # csv ファイルに保存。同じ名前のファイルがあるときは上書きされるため注意

# テストデータにおける、クラスごとの確率と推定結果の確認
y_test = pd.DataFrame(y_test)  # Series 型のため、行名と列名の設定は別に
y_test.columns = ["actual_class"]
estimated_y_test_for_check = pd.concat(
    [estimated_y_test_probability, y_test, estimated_y_test], axis=1
)  # 結合
estimated_y_test_for_check.to_csv(
    "./output/estimated_y_test_for_check.csv"
)  # csv ファイルに保存。同じ名前のファイルがあるときは上書きされるため注意
